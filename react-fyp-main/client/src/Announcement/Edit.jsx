...
export default function Edit() {
    ...
    useEffect(() => {
        if (announcement) {
            form.setFieldsValue({
                ...
                announcement: announcement.Announcement_Content
            });
    ...
    const handleSubmit = (values) => {
        confirm({
            ...
            onOk() {
                ...
                formData.append('Announcement_ID', alldonateditemID);
                formData.append('Announcement_Title', values.title);
                formData.append('Announcement_Content', values.announcement);
                if (fileList[0]?.originFileObj) {
                    formData.append('Announcement_Image', fileList[0].originFileObj);
                }

                axios
                    .post('/AdminEditAnnouncement', formData, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`
                        }
                    })
    ...
    return (
        <>
            ...
            <Title level={3} ><b>Edit Announcement ({alldonateditemID})</b></Title>
                ...
                <Row gutter={16}>
                    <Col span={6}>
                        ...
                        >
                            ...
                            >
                                {fileList.length >= 1 ? (
                                    <img src={URL.createObjectURL(fileList[0].originFileObj)} alt="Announcement" style={{ width: '100%' }} />
                                ) : announcement.Announcement_Image ? (
                                    <img src={buildAssetUrl(announcement.Announcement_Image)} alt="Announcement" style={{ width: '100%' }} />
                                ) : (
                                    uploadButton
                                )}
                            </Upload>
